...
class PartnerCharacteristics(models.Model):
    ...
    time_code = fields.Char(
        help='Specific code related to time or scheduling.'
    )

    who_cuts = fields.Char(
        string='Cutter/Responsible',
        help=('Indicates who is responsible for a specific '
    ...
    truck_unit = fields.Char(
        help=('Identifier for the truck or transport unit '
                'associated with the contact.'
            )
    )

    channel_code = fields.Char(
        help=('Code indicating the sales or distribution '
